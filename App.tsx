...
const AlbumPage = () => {
  ...
  useEffect(() => {
    if (id) {
      api.getAlbumById(id).then(setAlbum);
      api.getSongsByAlbum(id).then((s) => setSongs(s || []));
    }
  }, [id]);
...
const PlaylistPage = () => {
  ...
  useEffect(() => {
    if (id) {
      api.getPlaylistById(id).then(setPlaylist);
    }
  }, [id]);

  useEffect(() => {
    if (playlist?.songs) {
      api.getSongsByIds(playlist.songs).then((s) => setSongs(s || []));
    }
  }, [playlist]);

  // Reuse AlbumDetail for playlist view for now, or create a PlaylistDetail
  // Since AlbumDetail expects an Album object, we can adapt Playlist to Album-like structure
  // or just use a similar layout.
  // For simplicity, we'll adapt it.
  if (!playlist) return null;

  const playlistAsAlbum: Album = {
    id: playlist.id,
    name: playlist.name,
    image: playlist.image || "https://picsum.photos/400/400", // Default image
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col min-w-0">
                            ...
                            <span className="text-sm text-zinc-400 truncate group-hover:text-zinc-300">
                              {song.artist || "Rupam Islam & Fossils"}
                            </span>
                          </div>
                        </div>
...
const Home = ({
  ...
  return (
    <>
      <Hero onStartListening={onStartListening} />
  ...
  return (
    <div className="flex flex-col min-h-dvh bg-black text-white antialiased">
      <div className="flex flex-1">
        <button
          className="sm:hidden fixed top-4 left-4 z-50 p-3 bg-black/50 backdrop-blur-md rounded-full border border-zinc-800 text-white shadow-lg hover:bg-black/70 transition-all"
